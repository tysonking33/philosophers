...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/12 15:11:25 by tytang            #+#    #+#             */
/*   Updated: 2023/01/12 15:15:20 by tytang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_all(t_rules *gen, int argc, char **argv)
{
	...
	if (argc == 6)
		...
		gen->max_meals = 9999;
	if ((gen->phil_num < 1) || (gen->phil_num > 200)
		|| (gen->mill_sec_to_die < 60) || (gen->mill_sec_to_eat < 60)
		|| (gen->mill_sec_to_sleep < 60))
